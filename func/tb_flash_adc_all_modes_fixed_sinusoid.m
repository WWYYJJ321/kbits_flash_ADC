%% tb_flash_adc_all_modes_fixed_sinusoid.m
%%%% 修正版统一 TB（正弦波输入版）
%%%%
%%%% 功能：
...
% ===================== 新的正弦波输入 =====================
v_in_dyn = v_ref * sin(2 * pi * t_dyn);  % 幅度 1V，直流偏置 0
...
sgtitle('Flash ADC 4 Modes - Static Missing Code / DNL / INL + Dynamic Sinusoid Waveform', ...
...
for offset_selection = 1:4
    ...
    if ~is_meta_mode
        ...
        for idx = 1:N_STATIC
            code_rep = zeros(1, N_REPEAT_META);
            for r = 1:N_REPEAT_META
                [code_rep(r), ~] = kbits_flash_adc_leftrise_offset( ...
                    v_in_static(idx), v_ref, k_bits, vos_arr, offset_selection, vos_sigma);
            end
            code_rep = round(code_rep);
            code_rep = min(max(code_rep, 0), n_codes - 1);
            code_static(idx) = mode(code_rep);
        end
    end

    code_static = round(code_static);
    code_static = min(max(code_static, 0), n_codes - 1);
    v_out_static_plot = -v_ref + code_static * delta;

    %% -----------------------------------------------------
    % 5.4 动态时域数据（不变，只是输入变成正弦）
    %% -----------------------------------------------------
    ...
    %% -----------------------------------------------------
    % 5.6 计算 Missing Code / DNL / INL（不变）
    %% -----------------------------------------------------
    ...
    nexttile;
    hold on;
    N_show = round(N_DYNAMIC / ramp_periods * 1.5);  % 显示 1.5 个周期
    ...
    xlabel('Time (s)');
    ylabel('v_{in} / v_{out}');
    legend('Input Sinusoid', 'Output ADC', 'Location', 'best');
    ylim([-v_ref - 0.2, v_ref + 0.2]);
    title('Time Domain Sinusoid Waveform');
